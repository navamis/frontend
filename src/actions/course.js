const saveCourse = () => {
  return {
    type: "ADD_COURSE",
    payload: { message: "Successfully added course" },
  };
};
...
export const addCourse = (payload) => {
  ...
  return (dispatch) => {
    fetch("http://localhost:8075/courses", requestOptions).then((res) => {
      console.log(res);
      if (res.status === 201) {
        console.log("success");
        dispatch(saveCourse());
...
export const removeCourse = (course) => {
  return {
    type: "DELETE_COURSE",
    payload: { message: "Successfully deleted", course: course },
  };
};
...
export const deleteCourse = (courseId) => {
  ...
  return (dispatch) => {
    // let message = ''
    fetch(
      "http://localhost:8075/deleteCourseById/" + courseId,
      requestOptions
    ).then((res) => {
      if (res.status === 200) {
