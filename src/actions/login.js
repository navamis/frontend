const saveLogin = () => {
  return {
    type: "ADD_LOGIN",
    payload: { message: "Successfully added login" },
  };
};
...
export const addLogin = (payload) => {
  ...
  return (dispatch) => {
    fetch("http://localhost:8075/login", requestOptions).then((res) => {
      console.log(res);
      if (res.status === 201) {
        console.log("success");
        dispatch(saveLogin());
...
export const ChangePassword = (user) => {
  ...
  return (dispatch) => {
    fetch(
      "http://localhost:8075/loginchangePassword/" + user.userId,
      requestOptions
    ).then((res) => {
      console.log(res);
...
export const deleteLogin = (userId) => {
  ...
  return (dispatch) => {
    fetch("http://localhost:8075/login/" + userId, requestOptions).then(
      (res) => {
        if (res.status === 200) {
          dispatch(fetchLogin());
