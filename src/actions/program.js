const saveProgram = () => {
  return {
    type: "ADD_PROGRAM",
    payload: { message: "Successfully added program" },
  };
};
...
export const addProgram = (payload) => {
  ...
  return (dispatch) => {
    fetch("http://localhost:8075/programs", requestOptions).then((res) => {
      console.log(res);
      if (res.status === 201) {
        console.log("success");
...
export const removeProgram = (program) => {
  return {
    type: "DELETE_PROGRAM",
    payload: { message: "Successfully deleted", program: program },
  };
};
...
export const deleteProgram = (programId) => {
  ...
  return (dispatch) => {
    fetch(
      "http://localhost:8075/deleteProgramById/" + programId,
      requestOptions
    ).then((res) => {
      if (res.status === 200) {
...
export const updateProgram = () => {
  ...
  return {
    type: "UPDATE_PROGRAM",
    payload: { message: "Successfully updated program" },
  };
};
